...
item_sizes: std.ArrayListUnmanaged(Vec2),
...
pub fn render(self: *List, area: Rect, frame: Frame, theme: display.Theme) !void {
    var cursor = area.min;
...
pub fn layout(self: *List, constraints: Constraints) !Vec2 {
    const self_constraints = Constraints.fromProps(self.layout_properties);
    const item_constraints = Constraints{
        .min_height = 0,
        .min_width = 0,
        .max_height = @min(self_constraints.max_height, constraints.max_height),
